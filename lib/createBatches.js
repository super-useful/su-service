...
*/
function * loadService (params, query, headers, service) {

  var data = (yield service.api(params, query, headers));
...
function formatBatchResponse (data) {
  ...
  return reduce(data, function (acc, service) {
    acc[service.alias || service.id] = service.data;
    return acc;
  }, {});
}
...
function createBatch (batchName, host, services, batchApis) {
  ...
  var batchServices = reduce(batchApis, function (acc, id, alias) {
    var service;
    if (services[id]) {
      service = {
        id: id,
  ...
  return function * (params, query, headers) {
    ...
    var reqs = yield map(batchServices, partial(loadService, params, query, headers));
    // WHY IS THIS HERE?
    // IT DOESN'T SEEM TO DO ANYTHING?
    // DO WE NEED THIS?
    //var batches = yield parallel(reqs, reqs.length);

    return formatBatchResponse(reqs);
  }
}
