...
function createEndpointConfig(baseHeaders, baseUrl, acc, config) {
  ...
  if (baseUrl.pathname !== 'undefined') {

    endpoint.url.pathname = baseUrl.pathname + config.url.pathname;
  }
  else {
    endpoint.url.pathname = config.url.pathname;
  }
  ...
  acc[config.id] = function* callEndpoint(params, query, headers) {
    ...
    if (params && typeof params === 'object') {
      uri.pathname = resolvePath(uri.pathname, params);
    }
...
function resolvePath(pathname, params) {
  ...
  pathname.split('/').forEach(function (component) {
    pathname = pathname.replace(component, encodeURIComponent(component));
  });
